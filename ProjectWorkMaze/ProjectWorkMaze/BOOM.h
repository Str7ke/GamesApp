#pragma once
HANDLE consoleBoom = GetStdHandle(STD_OUTPUT_HANDLE);
...
void Login() {
	system("cls");
	cout << "Name(less than 20 letters) : ", cin >> name;
}
	...
	for (int j = 0; j < areaY; j++)
	{
		...
		for (int i = 0; i < areaX; i++)
		{
			if (i == selectedindexboomX && j == selectedindexboomY) {
				SetConsoleTextAttribute(consoleBoom, 0x70);
				cout << areaVisible[j][i] ;
				SetConsoleTextAttribute(consoleBoom, 0x07);
			}
			else {
				cout << areaVisible[j][i] ;
			}
